...
//
using TLERP.StringFun01;
using TLERP.Cmn;
using TLERP.Class;

namespace TLERP
{
  public partial class frmSDIcbo1_2F_Master : Form 
  {
    // Example Bank Branch ID 2F = 2 Fields (one is Title other is additional = 2Fields)
        ...
        string pReplaceable = "")
    {
      ...
      fKeyFieldName = fMain[3];         // Key Field Name
      fKeyFieldType = fMain[4];     // Key Field Type   (Int, Long, etc)
      fHasAddr = fMain[5];          // Has Address Button
      // fMain[5] for Address Btn
      fParentTableName = fMain[6];  // Parent Table Name for Combo
      fParentKeyField = fMain[7];  // Parent Key Field Name for Combo  
      fCbo1ToolTip = fMain[8];      // String for tooltip of the combobox
      //
      ...
      textTitle2.MaxLength = Convert.ToInt16(fMaxLen[3]);
      // Marked Edit
    ...
    private void AtFormLoad()
    {
        ...
        toolTipCbo1.SetToolTip(textTitle, ("Required: Name or Title, Max. Length: " + textTitle2.MaxLength.ToString() + " Alphanumeric Characters"));
      ...
      try
      {
        ...
        else
        {
          if (Convert.ToInt64(mtextID.Text) > 0)    // Selected large int so that it may not conflict with int16, int32 etc
          {
            ...
            tSQL = "select top 1 ";
            tSQL += fField[4] + " as title,";               // 0-
            tSQL += fField[5] + " as stitle,";              // 1-
            tSQL += fField[6] + " as title2,";              // 2-
            tSQL += fField[7] + ",";                        // 3- Ordering
            tSQL += fField[8] + " as parent_id,";           // 4-
            tSQL += fField[9] + ", ";                       // 5- isdisabled
            tSQL += fField[10];                             // 6- isdefault
            if (fHasAddr == "1")
            {
              tSQL += ", " + "addr_uid";                    //7-
            }
            ...
            else
            {
              ...
              textTitle2.Text = dRow.ItemArray.GetValue(2).ToString(); // dtset.Tables[0].Rows[0][1].ToString();
              mtextOrdering.Text = dRow.ItemArray.GetValue(3).ToString(); // 
              int tPid = 0;
              tPid = ( dRow.ItemArray.GetValue(4) == DBNull.Value ? 0 : Convert.ToInt16(dRow.ItemArray.GetValue(4).ToString()) );
              if (tPid > 0)
              {
              ...
              if (fHasAddr == "1")
              {
                fAddrUID = dRow.ItemArray.GetValue(7) == DBNull.Value ? 0 : Convert.ToInt64(dRow.ItemArray.GetValue(7));
          ...
          } // end else Convert.ToInt64(mtextID.Text) > 0
          else
          {
            btn_Save.Enabled = false;
            btn_Delete.Enabled = false;
        ...
        if (fValR[i] == "R")
        {
          ...
          { 
            ...
            case 4 :
              ...
              break;
            case 5:
              if (textTitle2.Text.ToString() == "" || textTitle2.Text.ToString() == string.Empty)
              {
                //ErrrMsg += "'" + fFieldTitle[i] + "' is Empty or Blank ";
                ErrrMsg = StrF01.BuildErrMsg(ErrrMsg, "'" + fFieldTitle[i] + "' is Empty or Blank... " + "");
    ...
    private bool SaveData()
    {
      ...
      try
      {
          ...
          if (!fAlreadyExists)
          {
                ...
                tSQL += " , " + fField[4];                                            // title
                tSQL += " , " + fField[5];                                            // ST 
                tSQL += " , " + fField[6];                                            // Title2
                tSQL += " , " + fField[7];                                            // Ordering 
                tSQL += " , " + fField[8];                                            // Pid
          ...
          else
          {
                ...
                tSQL += ", " + fField[6] + " = " + StrF01.EnEpos(textTitle2.Text.ToString());
                if (mtextOrdering.Text.ToString() == null || mtextOrdering.Text.ToString().Trim() == "")
                {
                    tSQL += ", " + fField[7] + " = 0";
                }
                ...
                {
                    tSQL += ", " + fField[7] + " = " + mtextOrdering.Text.ToString();
                }
                tSQL += ", " + fField[8] + " = " + cboParentID.SelectedValue.ToString();
    ...
    private void btn_Address_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        frmAddress_Master Dlg_Address = new frmAddress_Master(
            ...
            textTitle.Text.ToString(),
            fAddrUID,
            "update " + fTableName + " set addr_uid = <<TOBEFillED>> where " + clsGVar.LGCY + " and " + fKeyFieldName + " = " + mtextID.Text.ToString()
            );
        Dlg_Address.ShowDialog();
    ...
    private void btn_Pin_Click(object sender, EventArgs e)
    {
        ...
        {
            btn_Pin.Text = "&Pin";
            btn_Pin.Image = TLERP.Properties.Resources.BaBa_tiny_pin;
        }
        else
        {
          btn_Pin.Text = "&Un-Pin";
          btn_Pin.Image = TLERP.Properties.Resources.BaBa_tiny_pinned;
        }
    }
