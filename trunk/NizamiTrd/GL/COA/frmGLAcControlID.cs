...
{
  public partial class frmGLAcControlID : Form
  {
    ...
    private void AtFormLoad()
    {
        ...
        mtFromID.Mask = "";
        mtFromID.HidePromptOnLeave = true;
        mtToID.Mask = "";
            ...
            else
            {
                if (Convert.ToInt64(mtextID.Text) > 0)    // Selected large int so that it may not conflict with int16, int32 etc
                {
                    ...
                    else
                    {
                        ...
                        {
                            mtFromID.Text = dRow.ItemArray.GetValue(2).ToString();
                        }
                        // To ID
                        if (dRow.ItemArray.GetValue(4) != DBNull.Value)
                        {
                            mtToID.Text = dRow.ItemArray.GetValue(4).ToString();
                        }
                        // Check Box
                        ...
                        if (dRow.ItemArray.GetValue(6) != DBNull.Value)
                        {
    ...
    private bool SaveData()
    {
      ...
      try
      {
          ...
          if (!fAlreadyExists)
          {
              ...
              tSQL += ", '" + mtFromID.Text.ToString() + "'";
              tSQL += ", '" + mtToID.Text.ToString() + "'";
              //  
          ...
          else
          {
              ...
              tSQL = "update " + fTableName + " set ";
                  ...
                  tSQL += ", " + fField[7] + " = '" + mtFromID.Text.ToString() + "'";
                  tSQL += ", " + fField[8] + " = '" + mtToID.Text.ToString() + "'";
    ...
    public void frmSDILookUp()
    {
        ...
        try
        {
            ...
            frmLookUp sForm = new frmLookUp(
                fKeyField,
                "ac_control_title,ac_control_st,ordering,ac_control_fromid,ac_control_toid", 
                fTableName, 
                "Ac Control ID",
                1, 
                "ID,Ac Control Title,Short,Ordering,From Ac ID,To Ac ID", 
                "10,20,10,10,15,15", 
                "T,T,T,T,T,T",
                true, 
                fJoin
