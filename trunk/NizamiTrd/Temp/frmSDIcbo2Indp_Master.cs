...
namespace NizamiTrd
{
  public partial class frmSDIcbo2Indp_Master : Form
  {
    ...
    private void ClearThisForm()
    {
      ...
      //
      cboSecondID.SelectedIndex = ClassSetCombo.Set_ComboBox(cboSecondID, fcboDefaultValue);
      //
      ...
      else
      {
        if (Convert.ToInt64(mtextID.Text) > 0)    // Selected large int so that it may not conflict with int16, int32 etc
        {
          ...
          else
          {
            ...
            cboSecondID.SelectedIndex = ClassSetCombo.Set_ComboBox(cboSecondID, tSecondID);
